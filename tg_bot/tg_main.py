from my_types import AnswerType, ButtonMenuActions
...
async def answer_question(update: Update, context: ContextTypes.DEFAULT_TYPE, debug=False):
    ...
    if debug: await context.bot.send_message(chat_id, text=str(answer.answer_code))
    ...
    match answer.type:
        ...
        case AnswerType.NUMBER:
            await context.bot.send_message(chat_id, text=str(answer.answer_result.iat[0, 0]), reply_markup=InlineKeyboardMarkup(main_button_menu))
        case AnswerType.TABLE:
            filename = f"{datetime.now().strftime('%d_%m_%Y_%H_%M_%S')}_{question.replace(' ', '_')}_{user_id}.png"
            drawer.save_table_image_to_file(answer.answer_result, filename)
            try:
