    ...
    def __init__(
        ...
        if schema == None:
            ...
            for schema in all_schemas:
                ...
                views = self._inspector.get_view_names(schema=schema) if view_support else []
                self._all_tables.update(tables + views)


            if include_tables:
                # Split table names into schema and table parts
                table_parts = (table_name.split('.') for table_name in include_tables)
            ...
            if ignore_tables:
                ignore_table_parts = (table_name.split('.') for table_name in ignore_tables)
            ...
            with engine.connect() as connection:
                for table_name in self._usable_tables:
                    query = text("SELECT table_schema FROM information_schema.tables WHERE table_name = :table")
                    query = query.bindparams(table=table_name)
                    result = connection.execute(query)
                    schemas = [row[0] for row in result.fetchall()]
                    
                    if len(schemas) > 1: 
                        logger.error(f"More than one schema found for table {table_name}", exc_info=True)
                        raise ValueError(f"Найдено более одной схемы для таблицы {table_name}") 
                    ...
                    if schemas:
                        ...
                        logger.error(f"Schema not found for table {table_name}", exc_info=True)
