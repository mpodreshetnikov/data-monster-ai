...
class CustomSQLDatabase(SQLDatabase):
    ...
    def __init__(
        ...
        self.include_schema,  self.include_tables = map(
            set, zip(*(table_name.split('.') for table_name in include_tables)))
        for schema in self.include_schema:
            tables = self._inspector.get_table_names(schema=schema)
            views = self._inspector.get_view_names(
                schema=schema) if view_support else []
            self._all_tables.update(tables + views)

        self._include_tables = set(
            self.include_tables) if self.include_tables else set()
        ...
        for schema in self.include_schema:
            tables = self._inspector.get_table_names(schema=schema)
            for table in tables:
                if f"{schema}.{table}" in include_tables:
